...
enum MagicConstants {
    static let isBullseyeCrewEnable = "isBullseyeCrewEnable"
    static let webka = "webka"
    static let food = "food"
    static let signalID = "b4a668ad-409c-4871-b7fa-685c24fea86e"
}
...
final class RemoteManager: ObservableObject {
    ...
    private func updateLocalValues() {
        self.isRemoteEnable = config.configValue(forKey: MagicConstants.isBullseyeCrewEnable).boolValue
        self.savedBaseURLString = config.configValue(forKey: MagicConstants.webka).stringValue
        self.salt = config.configValue(forKey: MagicConstants.food).stringValue
...
struct URLBuilder {
    ...
    static func buildTrackingURL(from response: MetricsResponse, bundleID: String, idfa: String?) -> URL? {
        guard let components = makeBaseComponents(from: response) else {
            return nil
        }
        ...
        finalComponents.queryItems = queryItems.isEmpty ? nil : queryItems
